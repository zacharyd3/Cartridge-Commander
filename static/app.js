...
function renderBackupPage(c){
  ...
  folCard.innerHTML = `
    ...
    <div class="section-sep"></div>
    <div id="backup-browser-path" class="text-sm text-muted mono mt8">${window.APP_CONFIG.backupRoot}</div>`;
...
async function ensureBackupBrowser(){
  if(!G.backupBrowser) G.backupBrowser = await api(`/api/browse?path=${window.APP_CONFIG.backupRoot}`);
}
...
function renderRestorePage(c){
  ...
  sessPick.style.cssText='margin-top:8px;display:none;';
...
async function loadRestoreIndex(vol, silent){
  ...
  if(sessContainer){
    if(sessions.length > 1){
      ...
      sessContainer.innerHTML = `<div class="sess-picker-label">Backup session</div>`
        + `<div class="sess-options">`
        + sessions.map((s,i)=>`<button type="button" class="sess-opt${i===latestIdx?' active':''}" data-session="${s}">
            <span class="sess-opt-dot"></span>
            <span class="sess-opt-icon">📁</span>
            <span class="sess-opt-name">${s}/</span>
            ${i===latestIdx?'<span class="sess-opt-badge">Latest</span>':''}
          </button>`).join('')
        + `</div>`;
      sessContainer.querySelectorAll('.sess-opt').forEach(btn=>{
        btn.onclick = () => {
          sessContainer.querySelectorAll('.sess-opt').forEach(b=>b.classList.remove('active'));
          btn.classList.add('active');
          navigateRestoreSession(btn.dataset.session);
        };
      });
      // Auto-navigate into the most recent session
      navigateRestoreIntoDir(sessions[latestIdx] + '/');
      return;
    } else {
...
async function ensureSchedBrowser(){
  if(!G.schedBrowser) G.schedBrowser = await api(`/api/browse?path=${window.APP_CONFIG.backupRoot}`);
  renderSchedBrowser();
}
...
async function loadSchedules(){
  ...
  const backupRunning = !!G.state?.backup_job?.running;
...
async function runSchedNow(id){
  ...
  // Optimistically update backup_job state and jump to the Backup page,
  // mirroring startBackup() so the user sees activity immediately.
  if(G.state) G.state.backup_job = {
    ...G.state.backup_job,
    running: true,
    status: 'selecting_tape',
        ...
        ${[
          ...
          <div class="form-group" style="margin-bottom:10px;">
            ...
            <div style="display:flex;gap:6px;align-items:flex-start;">
              ...
              <button class="btn sm" onclick="resetTemplate('${k}')" title="Reset to default" aria-label="Reset to default" style="flex-shrink:0;margin-top:1px;">↺</button>
            </div>
          </div>`).join('')}
        <div class="btn-row" style="margin-top:4px;">
    ...
    if(idx){
      ...
      if(sessions.length > 1 && banner && switchEl){
        ...
        switchEl.innerHTML = ordered.map(s => {
          ...
          return `<button type="button" class="sess-opt${isActive ? ' active' : ''}" data-session="${s}">
            <span class="sess-opt-dot"></span>
            <span class="sess-opt-icon">📁</span>
            <span class="sess-opt-name">${s}</span>
          </button>`;
        }).join('');
        switchEl.querySelectorAll('.sess-opt').forEach(btn => {
          btn.onclick = () => switchTapeSession(btn.dataset.session);
        });
...
function switchTapeSession(sessionName){
  G.tdCwd = sessionName + '/';
  // Update active state in the session picker
  const switchEl = $('td-session-switch');
  if(switchEl){
    switchEl.querySelectorAll('.sess-opt').forEach(b => {
      b.classList.toggle('active', b.dataset.session === sessionName);
    });
  }
  $('td-search').value = '';
  renderTapeDrawerFiles('');
